# All In One POS — Play Store kit

| Doc | Purpose |
|---|---|
| [ACCOUNT.md](ACCOUNT.md) | Play Console + package name |
| [SIGNING.md](SIGNING.md) | Upload keystore + `key.properties` |
| [VERSIONING.md](VERSIONING.md) | Shared with iOS — `pubspec` `+N` bumps |
| [LISTING.md](LISTING.md) | Store listing copy + assets |
| [DATA_SAFETY.md](DATA_SAFETY.md) | Play Data safety form answers |
| [INTERNAL_TEST.md](INTERNAL_TEST.md) | Internal testing track smoke |
| [SUBMIT.md](SUBMIT.md) | Production rollout |

Privacy policy (shared with iOS): https://legal.allinonepos.app  

Build: see deploy skill §6 and `flutter build appbundle --release --dart-define-from-file=env.local.json`.
