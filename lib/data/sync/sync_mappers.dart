...
// --- supplier_payments -------------------------------------------------------
final _supplierPayments = SyncTableDef(
  name: 'supplier_payments',
  toRemote: (db, id) async {
    final r = await (db.select(db.supplierPayments)
          ..where((t) => t.id.equals(id)))
        .getSingleOrNull();
    if (r == null) return null;
    return {
...
final _orders = SyncTableDef(
  ...
  upsertLocal: (db, m) async {
    ...
    await db.into(db.orders).insertOnConflictUpdate(OrdersCompanion(
          ...
          customerName: Value(m['customer_name'] as String),
          customerPhone: Value(m['customer_phone'] as String?),
          customerIp: Value(m['customer_ip'] as String?),
          deliveryAddress: Value(m['delivery_address'] as String?),
...
// --- equity_entries ----------------------------------------------------------
final _equityEntries = SyncTableDef(
  name: 'equity_entries',
  toRemote: (db, id) async {
    final r = await (db.select(db.equityEntries)
          ..where((t) => t.id.equals(id)))
        .getSingleOrNull();
    if (r == null) return null;
    return {
